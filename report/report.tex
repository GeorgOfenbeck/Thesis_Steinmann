...
compiles and starts the Measuring Core and finally processes the results. We
decided to use Java for this code. This provides the flexibility  

This should result in straight forward code
for controlling the measurements and, since the control code is written in Java, a minimal amount of new concepts has to be
learned.
...
During the invocation of \method{addRooflinePoint()} and the quantity measuring service a
...
And finally, the requested quantities were calculated.
...
from the \class{KernelBase}. The buffers are allocated and initialized in
initialize() and freed in dispose(). \method{getBuffers()} returns the buffers along with
