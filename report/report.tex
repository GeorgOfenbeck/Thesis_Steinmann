...
\subsubsection{The MeasuringCoreService and the MeasurementAppController}
The MeasuringCoreService and the MeasurementAppController provide the low level
control over the measuring core. The service contains the logic to prepare,
build and run the core, while the controller keeps track of the state of the
core.

\umlDiagram{measurementDriver/MACMeasuringCoreService}

The measuring core service should only be used through application controller.
Since it makes sense that other services can start measusurements, the MeasurementService
provides a measure() method. It knows an instance of IMeasurementFacility, which
is implemented by the application controller and thus forwards all calls to
measure() to the app controller.

\umlDiagram{measurementDriver/measureCallLift}

Now, let's look at the measurement application controller.  This is the call
graph of the methods within the class:

\umlDiagram{measurementDriver/MACCallGraph}

Only the measure() method is public, the others are private. 
