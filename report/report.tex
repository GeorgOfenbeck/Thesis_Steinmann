...
The first series of measurements is about the running time of a CPU intensive kernel. We are not interested in the effects of the memory subsystem. We measure at both the minimal and maximal CPU frequency. We measure on an idle system, when another arithmetic kernel runs or when heavy disk IO is performed. We let the system load threads run on any of the system's CPU. We expect to measure effects of the task switches and interrupts due to IO.

The second series of measurements is about the influences of shared memory bandwidth on the runtime. Depending on the system architecture, we expect to measure shared bandwidth effects. We measure a memory load and a memory write kernel, with the same kernels running on other CPUs at the same time. 

Unfortunately, even on a very lightly loaded system, any task can be executed on any other core. This could affect short running kernels, and result in wrong measurement results. We show this by starting two memory intensive, short running kernels simultaneously. To overcome this, we propose to make sure all unused cores execute non memory intensive kernels during the measurement.

For long running measurements, it is sufficient to make sure that the average system load is low. The side effects of code running on other kernels will be smoothed out. This will be shown on various average loads (none, 1 2 3 5 10 20 40 50 75 100)
...
The first series of measurements is about the overhead of task switches on the measured memory transfer. We measure an arithmetic kernel and mem load and write, with buffer sizes which fit into the cache and which do not fit. We measure on an idle system, and with an arithmetic kernel on any CPU.

The second measurement series is about the separation of measuring transfer volume of different threads. We measure mem load and write with large buffers. We measure on an idle system and with mem kernels on the other CPUs.
