...
\subsection{Cache}
\graphFloat{yonah/FFTwarm}{Influence of the initial cache state}
...
the transfer volume should be constant, while the operation count grows with the
vector size. At some point the vectors are too large to fit into do not fit into
the caches into the
...
for the multi threaded MKL implementation.

\graphFloat{yonah/dgemvWarm}{Roofline Plot of the Matrix-Vector Multiplication
(MemL2, DoublePrecisionFlop)} 
Bringing the data into the caches before starting the measurements has an
influence for $n<=500$. 
...
The tripple loop has a poor performance, even for small matrices. This is
problably due to it's lack of unrolling. For matrix sizes exceeding $n=400$ the
drop in operational intensity is clearly visible.
