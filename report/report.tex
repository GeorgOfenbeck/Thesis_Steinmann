...
During the development of measurement control routines, it is expected that many changes do not affect the parameter points. To speed up repeated measurements after every minor change to the measurement driver, the measurement results are cached.

For convenience, a frontend program is provided. It compiles the measurement driver and executes it. It can be started using a shell script called "rmt", which has to be placed manually in a directory which is in the shell path.
...
\umlFloat{CentralComponents}{Central classes of the measuring core}
