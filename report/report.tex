...
But as soon as the kernels cause memory transfer, we observe a high variance
when measuring execution time and transferred bytes. We presume this is due to
various activities happening in the background during the measurement. (Task
switches etc). 
...
measured execution time, memory transfer or operation count. Ideally, the true
value is the minimal value observed. We therefore repeat each measurement $K$
times and discard all but the minimal value. 
