...
It was tried to follow best engineering practices for the design of the measurement driver. Unit tests (junit), mocks (jmock), and dependency injection  (guice) are used. Furthermore, a domain model (DOM), controllers and stateless services are used as described in \cite{evans2004domain}. Describing all these concepts lies beyond the scope of this report. It is assumed that the reader has a basic understanding of the named concepts.
...
In the measurement driver, each macro is identified by a macro key, which contains the macro name, a description and the default value. Macro keys should be declared in public static variables. Macro definitions are associated with a measurement definition. If no definition is given for a macro, the default definition found in the macro key is used.
