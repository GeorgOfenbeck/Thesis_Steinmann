...
sustems. A cache is a relatively small memory area which has a low access
...
can be held in the CPU caches, off-chip bandwith will not be an issue. 
...
transformantion: $M_{ij}=3j^2M_{ij}$.
...
Since both variants operate close to the peak memory bandwith, it is clear that
...
ridge point,  memory bandwith is not the limiting factor. If the kernel does not
