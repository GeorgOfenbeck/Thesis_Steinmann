...
The theoretical ceilings for pure additions is 1 flop/cycle and 0.5 flop/cycle.
...
\toprule
Operation&Instruction Set & DLP & UNROLL &  \multicolumn{2}{l}{Performance
[Flop/Cycle]} \\
\midrule
ADD   & x87       &  9 & 14 & 0&965\\
...
randomly accessed. The troughput is 0.269 bytes per cycle. Since a cache line is
64 bytes, this results in one memory access every $64/0.269=238$ cycles.

In our roofline plots we only show the load ceiling (2.2 flops per cycle) and
the random access ceiling (0.27 bytes per cycle).
