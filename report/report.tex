...
\subsection{Cache}
\graphFloat{yonah/FFTwarm}{Influence of the initial cache state (MemBus)}
...
Time})}\right)$} For the kernels causing memory transfer, we observe a
...
To investigate the influence of the initial cache state, measured a specific
implementation of a kernel with cold caches, with the data loaded into the
cahces (``-Data''), with the code loaded into the caches (``-Code'') and with
...
If only the code is initially loaded into the caches, the effect is relatively
small for small vectors and neglectable for larger vectors. (fig. \ref{yonah/daxpyWarm}) 

If the data is initially loaded into the caches, the operational intensity grows
with the size of the vector. The code has to be loaded once from memory. Thus
the transfer volume is constant, while the operation count grows with the
vector size. At some point the vectors are too large to fit into the cache and
the effect disappears. Theoretically the drop of operational intensity should
occur at a vector size of 131072 ($2MB/(2*8)$), but it already starts at a size
of 16000. The two double vectors of this size occupy one eight of the cache.
Since the L2 cache is 8 way associative, this is exactly the size wich fits into
the cache without using one cache line set twice. We did not investigate this
further.

If both data and code are initially loaded into the caches, the operational
intensity is theoretically infinite (or undefined), since no memory transfer is
required.

When the data is initally loaded into the caches a reduced precision is
observed. Since the measurements run for a relatively long period of time while
causing a small transfer volume, auxiliary data transfers might be measured
which occur at a fluctuating rate. This could cause the loss of precision.
...
What has been said for the vector-vector multiplication
about the effects of bringing data or code (or both) into the caches before
starting the measurement also applies to the matrix-vector multiplication,
although the numbers changed.

When loading the data initially into the caches, the operational intensity
starts to drop at a vector size of $n=300$. At this point the matrix occupies
$300^2*8=720000$ bytes. 
...
We used the implementation of the SPIRAL project \cite{Spiral}. Figure ? shows
the roofline plot. 
...
\cite{gnuplot} script and the required data file will be created in the current
