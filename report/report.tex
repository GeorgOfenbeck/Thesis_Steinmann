...
\newcommand{\file}[2]{\marginpar{\footnotesize #2}\index{referencedFiles}{#1}}
...
Each measurement can be performed with different compiler optimization flags and macro definitions. Therefore, the measuring core has to be rebuilt for each measurement, which makes rebuilding the measuring core a frequent operation. It should therefore be as fast as possible. This is archived by carefully tracking all build dependencies and by using ccache. 
...
\file{measuringCore/makefile}{measuringCore/\-makefile}
The build process is controlled using the make program \cite{make}.
