...
\begin{description}
\item[Level 1] Vector-Vector Multiplication $\bf y \leftarrow \alpha
\bf x + \bf y$
\item[Level 2] Matrix-Vector Multiplication $\mathbf y \leftarrow \alpha 
A \mathbf x  + \beta \bf{y}$
\item[Level 3] Matrix-Matrix Multiplication $C \leftarrow \alpha A B + \beta C $
\end{description}

There are various implementations available. We chose the Intel Math Kernel
Library (MKL) and the OpenBlas implementation. For each operation, figures
\ref{yonah/daxpy}, \ref{yonah/dgemv} and \ref{yonah/mmm} show the results for
