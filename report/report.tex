...
following paragraphs we will analyze the tripple loop and the blocked version.
To simplify matters, we will ignore minor effects.

The tripple loop uses the following algorithm:
...
the operational intensity is optimal ($n/16$). As soon as $B$ does not fit into
the cache it loaded over and over. In addition, this trashes the the write
back cache. The resulting memory transfer volume is $(n^3+3n^2)*8$ and, for
large matrices, the operational intensity is of $1/4$. This bound is nicely
visible on figure \ref{yonah/mmm}.
...
We chose $Nb=50$ and $Nu=Mu=Ku=2$.
For our discussion, only the blocking for $Nb$ is relevant.
