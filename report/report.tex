...
squeezed out of the cache by intermediate data accesses.
...
intensity' to mean number of performed operations per byte transferred between
CPU and memory. The exact definition of operation is problem specific, but
floating point operation is often used.
...
measurement results in a point on the plot. Multiple points are connected with a
line to a series. The graph is on a log-log scale.
...
What about memory bandwidth? If the implementation of an algorithm transfers $v$
bytes (transfer volume) and performs $\Omega$ operations within $t$ cycles (the
unit of time does not matter, could be seconds as well), it will be placed at
$x=\Omega/v$ (operational intensity) and $y=\Omega/t$ (performance). The data
throughput between CPU and memory is $\tau=v/t$, which {\bf can be derived from
...
Since roofline plots are on a log-log scale, the bandwidth line appears as a
line of unit slope, offset towards the top left of the graph proportional to the
logarithm of the memory bandwidth.
...
We measured the two variants with $m=1e6$ and $n=4$ on a random computer and got
...
Multi threaded kernels should be supported.

To get the data points for a kernel, we need to measure the performance and the
...
For example, if different implementations of an algorithm solve the same problem
with a different number of floating point operations, their floating point
performances are not comparable. It would make more sense to derive the
theoretical floating point operation count given the problem size and use the
result instead of measuring the actual operation count.
Otherwise just including dummy operations would increase the apparent
performance, which is certainly not desirable.
...
much data per unit of time or to execute as may operations per unit of time as
possible. Thus the measurement capabilities needed to evaluate the actual
problems can be reused for the benchmarks.
...
In addition we will present detailed low level measurement results, which could
be used as a reference for future work.

Finally, we implemented a variety of kernels (in many cases using libraries) and
present their roofline plots.
...
reading, we expect to observe the whole buffer being transferred to the core.
...
expected memory transfer for large buffer sizes.
...
Based on these considerations, we repeat each measurement $K$ times and
