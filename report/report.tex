...
cache. It is hard to estimate the number of writes which is actually held back,
therfore we just presume that the whole cache is used for write back. 
...
The first two graphs show the result of measuring the execution time of the ADD
kernel. Figure \ref{yonah/valTimeArithValues} shows that the execution time is
around one cycle per operation and does not differ between SSE and x87, which is
consistent with the information found in the Intel manual. Figure
\ref{yonah/valTimeArithError} shows that the errors are small for operation counts above 10000.

\graphFloat{yonah/valTimeMemValues}{Execution Time of the memory kernels} sdf

\graphFloat{yonah/valTimeMemError}{Error of the Execution Time of the memory kernels}

For the memory intesive kernels, we show the expected memory transfer on
the x axis. (fig. \ref{yonah/valTimeMemValues}) 

\graphFloat{yonah/dgemv}{bar}
\clearpage
