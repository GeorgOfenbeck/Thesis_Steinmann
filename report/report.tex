...
variant uses 64*'coreduo::BUS\_TRANS\_MEM', the {\bf MemL2} variant uses the
counters for the L2 cache misses and line eviction, namely
64*('coreduo::L2\_LINES\_IN:SELF'+'coreduo::L2\_M\_LINES\_OUT:SELF')
...
Using this scheme with $K=10$ for measuring the transferred bytes, we get four
error regions for different tranfer volumes: (fig. \ref{yonah/valTBMinError})
\begin{center}
\begin{tabular}{lrrrr}
\toprule
&\multicolumn{4}{c}{Region}\\
&$<$10KB&10KB-500KB&500KB-10MB&$>$10MB\\
\midrule
Read&5-25\%&5-10\%&5-40\%&0-5\%\\
Write&$>$5\%&5-30\%&10-50\%&5-10\%\\
Triad&5-30\%&10-15\%&10-15\%&0-5\%\\
\bottomrule
\end{tabular}
\end{center}

The accuracy and precision strongly depends on the problem size and the kernel.
For all kernels, the error lies under 5\% when the problem size exceeds 10MB.
For smaller problem size, we generally measure an overhead of 10\% when memory
writes are involved and 5\% for pure reading. When the transferred volume is
about 2MB, the overhead for the write and the read kernel has a peak and the
results are scattered.

The execution time of the read kernel is scattered within about 15\% of the
observed value. (fig. \ref{yonah/valTimeMemMinError}) For the write and the
triad kernel, values are far more scattered.
...
generally offset by about 10\% and scattered up to twice the minimum value.

\section{Multi Threaded Validation and Reproducibility}
...
each core. Since the Yonah does not utilize HyperThreading, we don't expect
...
\graphFloat{yonah/valTBThReadValues}{Yonah: Transfer Volume of the Read Kernel}
\graphFloat{yonah/valTBALTThReadValues}{Yonah: Transfer Volume of the Read
Kernel} 

\graphFloat{yonah/valTBThWriteValues}{Yonah: Transfer Volume of the Write
Kernel}
\graphFloat{yonah/valTBALTThWriteValues}{Yonah: Transfer Volume of the Write
Kernel}

\graphFloat{yonah/valTBThTriadValues}{Yonah: Transfer Volume of the Triad
Kernel}
\graphFloat{yonah/valTBALTThTriadValues}{Yonah: Transfer Volume of the Triad
Kernel}
