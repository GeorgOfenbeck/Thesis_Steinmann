...
of memory transfer. Around an expected transfer volume of 2MB we underestimate
the actual transfer volume. The CPU seems to write data back to the memory
before the cache is completely filled. The BusMem and BusL2 variants produce
similar result. Note that the BusL2 variant measures less than the expected
memory tansfer for large buffer sizes.

The errors (see fig. \ref{yonah/valTBError}) of the BusL2 variant are
smaller than those of the BusMem variant.
...
\graphFloatTwo{Error of Transferred Bytes with $K=10$}
...
writes. The huge differece in precision between MemBus and MemL2 disappeared
...
Applying the same scheme to the memory kernels does not yield good results. 
(fig. \ref{yonah/valTimeMemMinError}) Revisiting figure
\ref{yonah/valTimeMemValues}, 

The execution time of the read kernel is scattered within about 15\% of the observed value. (fig. \ref{yonah/valTimeMemMinError}) For the write and the
