...
Our kernels and benchmarks may use multi threading. This has to be supported by the measurement tool. Since some caches as well as part of the memory bandwith might be shared among different cores, multithreading can have various effects on the amount of transferred memory and the available bandwith.
...
It is possible that the operating system moves a thread from one processor to the other. Special care has to be taken to deal with this event. At least it has to be detected.
