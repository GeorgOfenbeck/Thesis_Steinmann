...
\newcommand{\graphNoVskip}[1]
{
	...
	\setlength{\imgwidth}{1.18\textwidth}
	\hfuzz50pt
...
before the cache is completely filled. The MemBus and MemL2 variants produce
similar result. Note that the MemL2 variant measures less than the expected
...
using the Intel Math Kernel Library (MKL) and the OpenBlas implementations.
...
vector element tripple, two operations are performed. Since the vector sizes
...
$n\times n$ matrix, the operation count is $2n$ for the two scale operations
...
$\max(n^2*8-k,0)$ bytes to store the result, where $k$ is the size of the last
level cache. 

The maximal operational intensity is $(2n^3+2n)/(3n^2*8+\max(n^2*8-k,0))$.
For large $n$ this becomes $(2n^3+2n)/(4n^2*8)\approx n/16$.

This operational intensity cannot be reached since reuse is bound by the cache
size. For the tripple loop, if the matrix fits completely into the cache the
operational intensity is the optimal 

\ref{yonah/mmm} show the results for
both libraries, both single and multi threaded. For the matrix-matrix
multiplication we also show the results of the tripple loop, blocking with
$Nb=50$ and $Nu=Mu=Ku=2$ and blocking with scalar replacement (MMM-Restrict).

TODO: exact definitions, what is tripple loop, blocking, sizes of the matrices
